            ...
            async with session.begin():
                await session.execute(
                    text("""
                         ...
                         VALUES (:id, :tenant_id, :question, :student_id, :confidence, 'pending') ON CONFLICT DO NOTHING
                         """),
                    {
                        "id": str(uuid.uuid4()),
                        "tenant_id": state["tenant_id"],
                ...
                async with session.begin():
                    await session.execute(
                        text("""
                             ...
                             VALUES (:id, :tenant_id, :student_id, :thread_id, :summary, 1) ON CONFLICT (thread_id) DO
                             UPDATE
...
if __name__ == '__main__':
    # print(_rule_classify_general(query="你是谁"))
    # print(asyncio.run(_determine_rag_strategy_fast(query="Transformer的架构组成是什么")))
    #
    # state = {"messages": [HumanMessage(content="请帮我概括下上述的知识")],
    #          "student_id": "stu_001",
    #          "session_id": "sess_001"}
    ...
    # async def main():
    #     state = {"original_query": "商品聚合这个项目适合的人群是谁",
    #              "course_id": "3e76aeed-5e01-4aa7-be8d-2055d12b9ea7",
    #              "tenant_id": "tenant_default",
    #              "query_type": "PRECISE", }
    #     result1 = await retrieve_node(state)
    #     print("*" * 80)
    #     state.update(result1)
    #     result2 = await generate_rag_node(state)
    #     print(result2)
    #
    # asyncio.run(main())

    # async def main():
    #     state = {"original_query": "什么是AI",
    #              "course_id": "748e565b-eafb-400a-8b22-66b4fa6007a2",
    #              "tenant_id": "tenant_default",
    #              "query_type": "PRECISE", }
    #     # result1 = await retrieve_node(state)
    #     # print("*" * 80)
    #     # state.update(result1)
    #     result2 = await web_search_node(state)
    #     print(result2)
    #
    #
    # asyncio.run(main())
    ...
    #              "query_type": "PRECISE", }
    #
    #     result1 = await web_search_node(state)
    #     state.update(result1)
    #     result2 = await generate_direct_node(state)
    ...
    state = {"query_type": "PRECISE",
             ...
             "tenant_id": "tenant_default",
             "student_id": "d73d42a4-9b86-4c25-a980-aff2ccf69639",  # 必须
             "confidence": 0.74}
