...
if __name__ == '__main__':
    ...
    #     resp = await llm.ainvoke([HumanMessage(content="用一句话介绍pyhton")])
    #     print("DeepSeek 回复:", resp.text)
    #     print("DeepSeek 回复:", resp.content)
    # asyncio.run(main())
    ...
    async def main():
        ...
        llm1 = get_structured_llm(agent_type="qa", output_schema=PersonInfo)
        messages = [
            SystemMessage(content="你负责从文本中抽取人物信息。"),
            HumanMessage(content="我叫小明，今年 25 岁，住在上海。"),
        ]

        result = await llm1.ainvoke(messages)
